...
class TestCompositeStock(unittest.TestCase):
    def test_is_composite_stock_true(self):
        ...
        self.assertTrue(get_fundamental_indicators.is_composite_stock(stock_info))
...
class Testget_good_stock_data(unittest.TestCase):
    def test_retrieved_good_stock(self):
        stock = get_fundamental_indicators.get_symbol_object("CSX", "NASDAQ")
        value = get_fundamental_indicators.get_good_stock_data(stock)
...
class Testget_write_symbol_to_csv(unittest.TestCase):
    ...
    def test_csv_retrieved_good_stock(self):
        value = get_fundamental_indicators.write_symbol_to_csv("CSX", "NASDAQ", cache=False)
        self.assertEqual(list(value.keys()), csv_cleaned_keys)
        ### CSV exists
        file_path = general.resultsPath("CSX_stock_data.csv")
        assert os.path.exists(file_path)
        ### cache exists
        value = get_fundamental_indicators.write_symbol_to_csv("CSX", "NASDAQ", cache=True)
        self.assertEqual(list(value.keys()), csv_cleaned_keys)
        file_path = general.dataPath("CSX_stock_data"+str(date.today())+".pkl")
        assert os.path.exists(file_path)
        ### correct keys
        self.assertEqual(list(value.keys()), csv_cleaned_keys)
    ...
    def test_csv_retrieve_good_stock_via_any_stock_option(self):
        value = get_fundamental_indicators.write_symbol_to_csv("CSX", "NASDAQ", cache=True, get_any_stock=True)
        self.assertEqual(list(value.keys()), csv_cleaned_keys)
        value = get_fundamental_indicators.write_symbol_to_csv("CSX", "NASDAQ", cache=False, get_any_stock=True)
        self.assertEqual(list(value.keys()), csv_cleaned_keys)
...
class Testget_write_symbols_to_csv(unittest.TestCase):
    ...
    def test_all_csv_retrieved_good_stocks(self):
        added=[{'symbol': "GOOG", 'exchange': "NASDAQ"},{'symbol': "CSX", 'exchange': "NASDAQ"},{'symbol': "CNI", 'exchange': "NYSE"},{'symbol': "CAT", 'exchange': "NASDAQ"},{'symbol': "V", 'exchange': "NYSE"},{'symbol': "^IXIC", 'exchange': "NASDAQ"},{'symbol': "^NYA", 'exchange': "NYSE"}]
        blacklist=[{'symbol': "GOOG", 'exchange': "NASDAQ"}, {'symbol': "DUK", 'exchange': "NYSE"}]
